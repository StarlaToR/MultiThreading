...
{
	//direction = ConvertAngleToDirection(GetAngleBetweenVectors(_direction, camForward));	

	if (_direction == Vec3(0, 0, 1))
		direction = Vec3(camForward.x, 0, camForward.z);

	else if (_direction == Vec3(0, 0, -1))
		direction = Vec3(-camForward.x, 0, -camForward.z);

	else if (_direction == Vec3(1, 0, 0))
		direction = Vec3(-camForward.z, 0, camForward.x);

	else if (_direction == Vec3(-1, 0, 0))
		direction = Vec3(camForward.z, 0, -camForward.x);
	
	AddVelocity(direction.GetNormalizedVector() * PLAYER_HORIZONTAL_ACCELERATION);
}
 
void Platformer::Player::Jump()
{
	AddVelocity(Vec3(0, PLAYER_VERTICAL_ACCELERATION, 0));		
	canJump = false;
	isGrounded = false;
...
void Platformer::Player::AddVelocity(Vec3 _velocity)
{
	...
	if (velocity.y > PLAYER_MAX_VERTICAL_SPEED)
		velocity.y = PLAYER_MAX_VERTICAL_SPEED;
	if (velocity.y < -PLAYER_MAX_VERTICAL_SPEED)
		velocity.y = -PLAYER_MAX_VERTICAL_SPEED;
